...
import com.knear.android.scheme.action.AddKeyAction
import com.knear.android.scheme.action.CreateAccountAction
...
open class NearService(
    ...
    private val jsonRpcProvider = JsonRpcProvider(rcpEndpoint)
    ...
    fun startLogin(publicKey: PublicKey, redirectUrl: String): Uri {
        ...
        return Uri.parse(walletUrl)
            ...
            .appendQueryParameter("public_key", publicKey.toString())
    ...
    fun sign(accountId: String, receiverId: String, amount: String): Uri {
        ...
        val account = keyPair?.let { Account(accountId, networkId, rcpEndpoint, it) }
        if (account != null) {
            val actions = listOf(AddKeyAction(123431L))
            return account.sign(receiverId, actions)
        }
        return Uri.parse(rcpEndpoint)
    }
    ...
    private fun getAccountKeyPair(accountId: String): KeyPairEd25519? {
        val androidKeyStore = AndroidKeyStore(this.sharedPreferences)
        val networkId =
            androidKeyStore.getNetworkId() ?: throw Error("sendMoney requires account logging")
